...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.color_paths, self.depth_paths, self.seg_paths, self.embedding_paths = self.get_filepaths()
    ...
    def get_filepaths(self):
        """Return paths to color images, depth images, semantic segmentation. Implement in subclass."""
    ...
    def _preprocess_seg(self, seg, shape):
        output_seg = []
        cv2_shape = (shape[1], shape[0])
        for key in [56, 57, 58, 59, 61, 62]:
            if seg[key] is not None:
                all_mask = np.zeros_like(seg[key][0])   
                for mask in seg[key]:
                    all_mask = np.logical_or(all_mask, mask)
                # resize to shape, in cv2 shape is (W, H)
                all_mask = cv2.resize(all_mask.astype(np.float32), cv2_shape, interpolation=cv2.INTER_NEAREST)
            else:
                all_mask = np.zeros(shape)
            output_seg.append(all_mask)
        result =  np.stack(output_seg, axis=0) # (6, H, W)
        if not self.channels_first:
            result = result.transpose(1, 2, 0) # (6, H, W) -> (H, W, 6)
        return result
    ...
    def __getitem__(self, index):
        ...
        shape = (self.desired_height, self.desired_width)
        if os.path.exists(seg_path):
            seg = np.load(seg_path, allow_pickle=True).item()
            seg = self._preprocess_seg(seg, shape)
        else:
            if self.channels_first:
                seg = np.zeros([6, shape[0], shape[1]])
            else:
                seg = np.zeros([shape[0], shape[1], 6])
